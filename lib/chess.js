...
exports.ChessGame = class ChessGame extends game.Game {
  constructor() {
    super();
    this._board = [
  ...
  isReadyToStart() {
    return (
      this._roleToPlayerId.hasOwnProperty(WHITE) &&
      this._roleToPlayerId.hasOwnProperty(BLACK)
    );
  }
  ...
  executeMove(move) {
    ...
    if (!this._playerIdToRole.hasOwnProperty(move.playerId)) {
      return { success: false, message: 'Player is not a participant.' };
    } else if (this._playerIdToRole[move.playerId] !== this._whoseTurn) {
	...
	return {
	  success: false,
	  message: 'You cannot move a piece from this position.',
	};
      }
	...
	if (srcPiece[1] === PAWN && srcPos.rank === 1 && dstPos.rank === 3) {
	  newEnPassantInfo = {
	    available: true,
	    dstPos: {rank: 3, file: dstPos.file},
	    capturePos: dstPos,
	  };
  ...
  _wouldEveryMoveResultInCheck(color, board, enPassantInfo, castlingInfo) {
    for (let rank = 0; rank < board.length; rank++) {
      for (let file = 0; file < board[file].length; file++) {
	const piece = board[rank][file];
	if (piece[0] !== color) {
	  continue;
	}
