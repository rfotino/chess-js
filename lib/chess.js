...
exports.ChessGame = class ChessGame extends game.Game {
  ...
  isReadyToStart() {
    return this._roleToPlayerId.has(WHITE) && this._roleToPlayerId.has(BLACK);
  }
  ...
  executeMove(move) {
    ...
    if (!this._playerIdToRole.has(move.playerId)) {
      return { success: false, message: 'Player is not a participant.' };
    } else if (this._roleToPlayerId.get(this._whoseTurn) !== move.playerId) {
  ...
  toObj(playerId = null) {
    ...
    if (playerId && this._playerIdToRole.has(playerId)) {
      obj.myRole = this._playerIdToRole.get(playerId);
    }
    return obj;
  }
